...
export default function RegisterPage() {
    ...
    const checkUrlErrors = useCallback(() => {
        ...
        if (errorParam) {
            if (errorCode === 'otp_expired') {
                setError('Ссылка для регистрации истекла');
    ...
    useEffect(() => {
        const checkUser = async () => {
            ...
            if (user) {
                console.log('[Register Page] User already authenticated, redirecting to /learn');
                router.push('/learn');
            }
        };
        ...
        const unsubscribe = onAuthStateChange(async (event, session) => {
            console.log('[Register Page] Auth state changed:', event, !!session);
            if (session?.user) {
                ...
                console.log('[Register Page] User registered, redirecting to /learn');
                router.push('/learn');
            }
        });
    ...
    async function handleGoogleSignUp() {
        ...
        if (error) {
            setError('Не удалось зарегистрироваться через Google. Попробуйте еще раз.');
            setLoading(false);
    ...
    async function createOrUpdateProfile(userId: string, email: string, provider: string) {
        ...
        try {
            ...
            if (!existingProfile) {
                ...
                console.log('[Register] Created new profile for user:', userId);
            } else {
                ...
                await supabase
                    ...
                    .eq('id', userId);
                console.log('[Register] Updated profile for user:', userId);
            }
        } catch (error) {
            console.error('[Register] Error creating/updating profile:', error);
        }
    }
